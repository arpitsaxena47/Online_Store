...
class FinalizeShoppingActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var calculateTotalPriceUrl = "http://192.168.43.29/OnlineStoreApp/calculate_total_price.php?" +
        ...
        var stringRequest = StringRequest(Request.Method.GET,calculateTotalPriceUrl,Response.Listener {
            response ->
            btnPaymentProcessing.text = "Pay Rs=$response via PayPal"
            ttPrice = response.toLong()
        },Response.ErrorListener {
            error ->

            val dialogBuilder = AlertDialog.Builder(this@FinalizeShoppingActivity)
